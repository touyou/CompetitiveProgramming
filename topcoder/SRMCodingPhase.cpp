...
public:
	int num[3];
	int solve(vi skills, int s, int luck, vi points) {
		num[0]=2;num[1]=4;num[2]=8;
		int res = 0;
		int ntime = 0;
		for (int i=s; i>=0; i--) {
			if (skills[i] <= luck) {
				...
				skills[i] -= luck;
				luck = 0;
				if (ntime+skills[i]<=75) {
					res += points[i]-skills[i]*num[i];
					ntime+=skills[i];
				}
			}
		}
   ...
   int countScore( vector <int> points, vector <int> skills, int luck ) {
		...
		if (luck==0) {
			...
			for (int i=0; i<3; i++) res=max(res,solve(skills,i,luck,points));
		}
		return res;
   }
};
