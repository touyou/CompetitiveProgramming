    ...
    int findMissing( vector <int> program, int wantedResult ) {
	  ...
	  for (int i=0; i<sz; i++) {
		if (program[i]==0||program[i]==-1) {
		  ...
		  st.push((ll)program[i]);
		}
	  }
	  if ((int)st.top()==wantedResult) return 0;
	  // else
	  ...
	  ll res = st.top();
	  if (iscons.top()) {
		if (wantedResult<=res) return -1;
		else return wantedResult-res;
	  } else {
		if (wantedResult==res) return 0;
		else return -1;
	  }
    }
};
